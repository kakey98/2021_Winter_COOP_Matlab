...
while (~feof(fid)) % 파일이 끝날 때까지 반복
    ...
    if (strcmp(str_read(1:3),'END')==1) %리스트 첫번째가 "END"면 루프문을 끊음
        break

    elseif (strcmp(str_read(1:4),'ATOM') && strcmp(str_read(14:15),"CA") && strcmp(str_read(22),"A")) % ATOM CA A chain 조건일 때 들어옴

        x = str2double(str_read(32:38)); % x coordinate info
        y = str2double(str_read(40:46)); % y coordinate info
        z = str2double(str_read(48:54)); % z coordinate info
        %string을 double로 바꿈
